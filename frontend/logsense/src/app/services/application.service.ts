...
export class ApplicationService {
  ...
  getApplicationByApplicationName(pc_id: number, application_name: string, start: string, end: string, bucket: string): Observable<Application> {
    const url: string = `http://localhost:8000/pc/${pc_id}/application/${application_name}?start=${start}&end=${end}&bucket_value=${bucket}`;
